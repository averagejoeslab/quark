...
| **Long-term memory** | `.quark/memory/memory.md` — "your memory extended into the world for persistence across sessions." Includes init, the format contract (bullets phrased for future grep-ability), the printf+heredoc write recipe, and reads framed as questions composed with any text tools over the contract — moves, not a menu. |
